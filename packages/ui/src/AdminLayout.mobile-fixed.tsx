...
const navigation: NavItem[] = [
  ...
  { name: 'Analytics', href: '/admin/analytics', icon: BarChart3 },
  {
    ...
    children: [
      ...
      { name: 'Files', href: '/admin/content/files', icon: FileText },
  ...
  },
  {
    ...
    children: [
      ...
      { name: 'Timelines', href: '/admin/world/timelines', icon: BookOpen },
      { name: 'Characters', href: '/admin/world/characters', icon: BookOpen },
      { name: 'Artist Collab', href: '/admin/world/artists', icon: BookOpen },
    ],
  },
  {
    name: 'Commerce',
    icon: ShoppingCart,
    children: [
      { name: 'Shop', href: '/admin/commerce/shop', icon: ShoppingCart },
      { name: 'Orders', href: '/admin/commerce/orders', icon: ShoppingCart },
      { name: 'Customers', href: '/admin/commerce/customers', icon: ShoppingCart },
    ],
  },
  {
    name: 'Beta',
    icon: Users,
    children: [
      { name: 'Applications', href: '/admin/beta/applications', icon: Users },
      { name: 'Managers', href: '/admin/beta/managers', icon: Users },
      { name: 'Activity', href: '/admin/beta/activity', icon: Users },
    ],
  },
  {
    name: 'Settings',
    icon: Settings,
    children: [
      { name: 'Users', href: '/admin/settings/users', icon: Settings },
...
export const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800 text-gray-100 relative">
      ...
      <div className="lg:hidden sticky top-0 z-[1000] bg-slate-800/95 backdrop-blur-md border-b border-amber-500/30 px-4 py-3 flex items-center justify-between shadow-lg">
        <div className="flex items-center space-x-3">
          {/* Mobile menu toggle - larger touch target */}
          <button
            onClick={toggleSidebar}
            className="p-3 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 hover:text-amber-200 transition-all duration-200 border border-amber-500/30 min-w-[48px] min-h-[48px] flex items-center justify-center"
      ...
      {isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[999] transition-opacity duration-300 touch-action: none"
          onClick={toggleSidebar}
      ...
      >
        <div className="h-full flex flex-col relative overflow-hidden">
          ...
          <div className="p-4 lg:p-6 border-b border-amber-500/30 bg-gradient-to-r from-slate-800/50 to-transparent relative flex items-center justify-between">
            ...
            <button
              onClick={toggleSidebar}
              className="p-2 lg:p-2 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg hover:bg-slate-700/50 text-slate-400 hover:text-amber-300 transition-colors duration-200"
                  ...
                  {item.children ? (
                    <div>
                      ...
                      <button
                        ...
                        className="w-full group flex items-center justify-between p-3 lg:p-3 rounded-xl text-slate-300 hover:bg-slate-700/30 transition-all duration-200 min-h-[52px] touch-action-manipulation"
                        ...
                        <ul className="space-y-1 ml-4 lg:ml-4 pb-2">
                          {item.children.map((child) => (
                            <li key={child.name}>
                              <NavLink
                                to={child.href}
                                onClick={() => isMobile && setIsSidebarOpen(false)} // Close menu on mobile nav
                                className={({ isActive }) =>
                                  cn(
                                    'group flex items-center p-3 lg:p-2 rounded-xl transition-all duration-200 relative overflow-hidden space-x-3 min-h-[48px] touch-action-manipulation',
                                    isActive
                    ...
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        cn(
                          'group flex items-center p-3 lg:p-3 rounded-xl transition-all duration-200 relative overflow-hidden space-x-3 min-h-[52px] touch-action-manipulation',
                          isActive
          ...
          <div className="p-3 lg:p-4 border-t border-amber-500/20 bg-gradient-to-r from-slate-800/30 to-transparent">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="p-2 lg:p-2 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg hover:bg-gradient-to-r hover:from-red-500/20 hover:to-red-600/10 text-slate-400 hover:text-red-300 transition-all duration-200 border border-transparent hover:border-red-500/30"
                title="Sign out"
                aria-label="Sign out"
              >
                <LogOut size={18} />
