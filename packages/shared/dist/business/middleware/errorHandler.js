    ...
    return (req, _res, next) => {
        try {
            ...
            if (!validation.isValid) {
                const error = new BaseError('Business rule validation failed', ErrorCode.BUSINESS_RULE_VIOLATION, 422, true, { violations: validation.errors });
                next(error);
                return;
            }
            next();
...
export const createRateLimiter = (windowMs, max, message) => {
    ...
    return (req, _res, next) => {
        ...
        if (userRequests.length >= max) {
            const error = new BaseError(message || 'Too many requests', ErrorCode.RATE_LIMIT_EXCEEDED, 429, true, { limit: max, windowMs, retryAfter: Math.ceil(windowMs / 1000) });
            next(error);
            return;
        }
