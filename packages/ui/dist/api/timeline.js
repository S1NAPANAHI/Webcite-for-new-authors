...
export const fetchTimelineEvents = async ({ page = 1, limit = 10, includeUnpublished = false, } = {}) => {
    ...
    const eventsWithNested = await Promise.all((data || []).map(async (event) => {
...
export const fetchTimelineEventById = async (id) => {
...
export const createTimelineEvent = async (eventData) => {
    ...
    return fetchTimelineEventById(newEvent.id);
};
export const updateTimelineEvent = async (id, eventData) => {
    ...
    return fetchTimelineEventById(id);
};
...
export const reorderTimelineEvents = async (reorderData) => {
    const updates = reorderData.map(({ id, order }) => supabase
        .from(TABLE_NAME)
        .update({ order })
        .eq('id', id));
...
export const toggleTimelineEventPublishStatus = async (id, isPublished) => {
