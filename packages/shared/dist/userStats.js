 ...
 */
const createDefaultStats = (userId) => ({
    user_id: userId,
    books_read: 0,
    reading_hours: 0,
    achievements: 0,
 ...
 */
const mapDbToAppStats = (dbStats, userId) => {
    if (!dbStats) {
...
export const getUserStats = async (userId) => {
    ...
    try {
        ...
        if (data) {
            const stats = mapDbToAppStats(data, userId);
...
export const initializeUserStats = async (userId) => {
    try {
        ...
        if (data) {
            const mappedStats = mapDbToAppStats(data, userId);
