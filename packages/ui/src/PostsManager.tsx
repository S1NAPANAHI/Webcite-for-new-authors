...
const createPost = async (newPost: Omit<Post, 'id' | 'created_at' | 'updated_at' | 'author_id' | 'views'>): Promise<Post> => {
  ...
  const { data, error } = await supabase.from('posts').insert({ ...newPost, author_id: user.id });
  if (error) throw new Error(error.message);
...
export const PostEditor = ({ post, onSave, onCancel }) => {
  ...
  const handleSubmit = () => {
    ...
    if (post) { // If editing an existing post, include the ID
      dataToSave.id = post.id;
    }
    // For new posts, 'id' is intentionally omitted as it's auto-generated by Supabase
