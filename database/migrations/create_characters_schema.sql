-- Character Management Schema for Zoroasterverse
...
CREATE TABLE characters (
  ...
  -- Story Integration
  first_appearance_content_id UUID REFERENCES content_items(id) ON DELETE SET NULL,
  last_appearance_content_id UUID REFERENCES content_items(id) ON DELETE SET NULL,
...
CREATE TABLE character_appearances (
  ...
  -- Prevent duplicate appearances
  UNIQUE(character_id, content_item_id, chapter_id)
);
...
CREATE TRIGGER update_character_abilities_updated_at BEFORE UPDATE ON character_abilities
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_character_versions_updated_at BEFORE UPDATE ON character_versions
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE POLICY "Public characters are viewable by everyone" ON characters
    FOR SELECT USING (NOT is_spoiler_sensitive OR auth.role() = 'admin');
...
CREATE POLICY "Public character abilities are viewable" ON character_abilities
    FOR SELECT USING (NOT is_hidden OR auth.role() = 'admin');
