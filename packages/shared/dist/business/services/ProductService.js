import { CreateProductSchema, UpdateProductSchema, ProductQuerySchema, validateProductBusinessRules, validatePriceBusinessRules } from '../validators/product.validator';
...
export class ProductService {
    ...
    async getProducts(query) {
        ...
        `);
            ...
            if (validatedQuery.search) {
                supabaseQuery = supabaseQuery.or(`name.ilike.%${validatedQuery.search}%,description.ilike.%${validatedQuery.search}%`);
            }
            // Apply sorting
     ...
     */
    async getProductsByType(productType, activeOnly = true) {
        try {
     ...
     */
    async validateProductAccess(productId, userId) {
        try {
    ...
    async getRecommendedProducts(userId, limit = 6) {
        try {
            ...
            const purchasedTypes = purchases?.map(p => p.products?.product_type).filter(Boolean) || [];
     ...
     */
    async updateProductAvailability(workId, workStatus) {
        try {
