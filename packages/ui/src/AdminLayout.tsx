...
const navigation: NavItem[] = [
  { name: 'Dashboard (New)', href: '/admin', icon: LayoutDashboard },
...
export const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800 text-gray-100">
      <h1 style={{color: 'red', fontSize: '48px'}}>HELLO FROM ADMIN LAYOUT</h1>
            ...
            <ul className="space-y-2">
              {navigation.map((item) => (
                <li key={item.name}>
                  {item.children ? (
                    <div>
                      <div className="group flex items-center p-3 rounded-xl space-x-3 text-slate-300">
                        ...
                        {item.children.map((child) => (
                          <li key={child.name}>
                            <NavLink
                              ...
                              className={({ isActive }) =>
                                cn(
                                  'group flex items-center p-2 rounded-xl transition-all duration-200 relative overflow-hidden space-x-3',
                                  isActive
                                    ? 'bg-gradient-to-r from-amber-500/20 to-yellow-500/10 text-amber-300'
                                    : 'text-slate-400 hover:bg-slate-700/50 hover:text-amber-300'
                                )
                              }
                            >
                              {child.name}
