...
const DualScrollingProphecy = ({ spinsLeft, setSpinsLeft }) => {
    const englishReelRef = useRef(null);
    ...
    return (_jsx(_Fragment, { children: _jsx("div", { className: `${styles.prophecyMask} ${styles.englishMask}`, onClick: handleSpin, children: _jsx("div", { ref: englishReelRef, className: styles.prophecyReel, children: reelItems.map((item, index) => (_jsx("div", { className: styles.prophecyItem, children: _jsx("span", { className: styles.englishText, children: item.english }) }, index))) }) }) }));
};
// --- UI COMPONENTS ---
const HeroSection = ({ contentMap, spinsLeft, setSpinsLeft }) => {
    ...
    return (_jsxs("section", { id: "home", className: styles.zrHero, children: [_jsxs("div", { className: styles.zrHeroContent, children: [_jsx("h1", { className: styles.zrTitle, children: title }), _jsx("p", { className: styles.zrQuote, children: quote }), _jsx("p", { className: styles.zrIntro, children: intro }), _jsx(Link, { className: styles.zrCta, to: "/blog/about", children: "Learn More" })] }), _jsxs("figure", { className: styles.zrHeroArt, "aria-labelledby": "art-caption", children: [_jsx("video", { src: "/200716-913538378.mp4", autoPlay: true, loop: true, muted: true, playsInline: true, className: styles.videoFire }), _jsx("div", { className: styles.spinsIndicator, children: [...Array(3)].map((_, i) => (_jsx("div", { className: `${styles.spinDot} ${i < spinsLeft ? styles.spinDotActive : ''}` }, i))) }), _jsx(DualScrollingProphecy, { spinsLeft: spinsLeft, setSpinsLeft: setSpinsLeft }), _jsx("figcaption", { id: "art-caption", className: styles.srOnly, children: "A stylized winged figure above a sacred fire." })] })] }));
};
const LatestPosts = ({ posts }) => {
    return (_jsxs("section", { className: styles.zrSection, children: [_jsx("h2", { className: styles.zrH2, children: "Latest News & Updates" }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-8", children: posts.map(post => {
                    const excerpt = post.content.length > 100 ? post.content.substring(0, 100) + '...' : post.content;
                    return (_jsxs("div", { className: styles.parchmentCard, children: [_jsx("h3", { children: post.title }), _jsx("p", { className: "mt-2", dangerouslySetInnerHTML: { __html: excerpt } }), _jsx(Link, { to: `/blog/${post.slug}`, className: "mt-4 inline-block", children: "Read More" })] }, post.id));
                }) })] }));
};
const LatestReleases = ({ releases }) => {
    return (_jsxs("section", { className: styles.zrSection, children: [_jsx("h2", { className: styles.zrH2, children: "Latest Releases" }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-8", children: releases.map(release => (_jsxs("div", { className: styles.parchmentCard, children: [_jsx("h3", { children: release.title }), _jsxs("p", { className: "mt-2", children: ["Type: ", release.type] }), _jsx("a", { href: release.link || '#', className: "mt-4 inline-block", children: "View Details / Purchase" })] }, release.id))) })] }));
};
// --- MAIN HOME PAGE COMPONENT ---
export const HomePage = () => {
    const { data: homepageData, isLoading: isLoadingHomepage, isError: isErrorHomepage } = useQuery({ queryKey: ['homepageContent'], queryFn: fetchHomepageContent });
    const { data: latestPosts, isLoading: isLoadingPosts, isError: isErrorPosts } = useQuery({ queryKey: ['latestPosts'], queryFn: fetchLatestPosts });
    const { data: releaseData, isLoading: isLoadingReleases, isError: isErrorReleases } = useQuery({ queryKey: ['releaseItems'], queryFn: fetchReleaseItems });
    ...
    if (isLoading)
        return _jsx("div", { className: "text-center py-8", children: "Loading homepage content..." });
    if (isError)
        return _jsx("div", { className: "text-center py-8 text-red-400", children: "Error loading homepage content." });
    const contentMap = new Map(homepageData?.map(item => [item.section, item]));
    return (_jsxs("div", { children: [_jsx(HeroSection, { contentMap: contentMap, spinsLeft: spinsLeft, setSpinsLeft: setSpinsLeft }), _jsxs("section", { className: styles.zrSection, children: [_jsx("h2", { className: styles.zrH2, children: "Our Progress" }), _jsxs("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-8 text-center", children: [_jsxs("div", { className: styles.parchmentCard, children: [_jsx("h3", { className: "text-4xl font-bold text-primary", children: contentMap.get('statistics_words_written')?.content || '0' }), _jsx("p", { className: "text-muted-foreground", children: "Words Written" })] }), _jsxs("div", { className: styles.parchmentCard, children: [_jsx("h3", { className: "text-4xl font-bold text-primary", children: contentMap.get('statistics_beta_readers')?.content || '0' }), _jsx("p", { className: "text-muted-foreground", children: "Beta Readers" })] }), _jsxs("div", { className: styles.parchmentCard, children: [_jsx("h3", { className: "text-4xl font-bold text-primary", children: contentMap.get('statistics_average_rating')?.content || '0' }), _jsx("p", { className: "text-muted-foreground", children: "Average Rating" })] }), _jsxs("div", { className: styles.parchmentCard, children: [_jsx("h3", { className: "text-4xl font-bold text-primary", children: contentMap.get('statistics_books_published')?.content || '0' }), _jsx("p", { className: "text-muted-foreground", children: "Books Published" })] })] })] }), _jsx(LatestPosts, { posts: latestPosts || [] }), _jsx(LatestReleases, { releases: releaseData || [] }), _jsxs("section", { className: styles.zrSection, children: [_jsx("h2", { className: styles.zrH2, children: "Artist Collaboration" }), _jsxs("div", { className: "relative rounded-lg shadow-lg overflow-hidden w-full", children: [_jsx("img", { src: "/images/invite_to_Colab_card.png", alt: "Artist Collaboration Invitation", className: "w-full h-full object-contain" }), _jsxs("div", { className: "absolute inset-0 bg-black bg-opacity-50 flex flex-col items-center justify-center p-8", children: [_jsx("h3", { className: "text-2xl font-bold text-white mb-4 text-shadow-md", children: "Join Our Creative Team!" }), _jsx("p", { className: "text-white mb-6 text-shadow-sm", children: "We're looking for talented artists to help shape the visual identity of the Zangar/Spandam Series. Explore revenue-share opportunities and bring your vision to life." }), _jsx(Link, { to: "/artist-collaboration", className: "inline-block bg-primary hover:bg-primary-dark text-white font-bold py-3 px-6 rounded-lg transition duration-300 ease-in-out transform hover:scale-105", children: "Apply Now" })] })] })] })] }));
};
