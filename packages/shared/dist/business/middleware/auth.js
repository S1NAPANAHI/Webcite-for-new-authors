...
export const authorizeAction = (action, getTargetUserRole) => {
    return async (req, res, next) => {
        try {
            ...
            const canPerformAction = BusinessRulesEngine.user.canPerformAction(req.user.role, action, targetUserRole);
            if (!canPerformAction) {
...
export const authorizeContentAccess = (supabase, contentType) => {
    return async (req, res, next) => {
        try {
            ...
            const canAccess = BusinessRulesEngine.content.canAccessContent(req.user.role, contentType, subscriptionStatus);
            if (!canAccess) {
 ...
 */
export const createUserRateLimiter = (baseWindowMs, getMaxRequests) => {
