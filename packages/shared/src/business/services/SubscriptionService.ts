...
import {
  ...
  validateSubscriptionBusinessRules
...
export class SubscriptionService {
  ...
  async createSubscription(input: CreateSubscriptionInput, _createdBy: string) {
    try {
      ...
      if (!plan.active || !(plan as any).products?.active) {
   ...
   */
  async cancelSubscription(id: string, cancelAtPeriodEnd: boolean = true, canceledBy: string, reason?: string) {
    try {
   ...
   */
  async reactivateSubscription(id: string, reactivatedBy: string) {
    try {
      const subscription = await this.getSubscriptionById(id);
   ...
   */
  private async getUserActiveSubscription(userId: string, planId?: string) {
    try {
   ...
   */
  private async validateSubscriptionLimits(userId: string) {
    try {
   ...
   */
  private async handleStatusChange(subscription: any, newStatus: SubscriptionStatus) {
    try {
   ...
   */
  private async grantSubscriptionEntitlements(subscriptionId: string, userId: string, planId: string) {
    try {
      // Get plan details
      const { data: plan, error: planError } = await this.supabase
        .from('prices')
   ...
   */
  private async revokeSubscriptionEntitlements(subscriptionId: string, userId: string) {
    try {
  ...
  async getSubscriptionAnalytics(startDate?: string, endDate?: string) {
    try {
      ...
      const statusBreakdown = statusCounts?.reduce((acc: any, sub) => {
   ...
   */
  async getUserSubscriptionHealth(userId: string) {
    try {
   ...
   */
  private calculateHealthScore(active: number, expiring: number, canceling: number): number {
