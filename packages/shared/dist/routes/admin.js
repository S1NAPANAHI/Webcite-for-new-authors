...
function createAdminRoutes(supabase) {
    ...
    router.get('/inventory/movements', async (_req, res) => {
        try {
    ...
    router.get('/analytics/sales', async (_req, res) => {
        ...
        `)
            ...
            const totalRevenue = completedOrders.reduce((acc, o) => acc + o.total_amount, 0);
    ...
    router.get('/webhooks/events', async (_req, res) => {
        try {
            ...
            if (status === 'success')
                query = query.eq('success', true).eq('processed', true);
            if (status === 'failed')
                query = query.eq('success', false).eq('processed', true);
            if (status === 'pending')
    ...
    router.get('/orders', async (req, res) => {
        try {
            const { limit = 20, offset = 0, status, search, dateFrom, dateTo } = req.query;
            let query = supabase
                .from('orders')
                .select(`
          *,
