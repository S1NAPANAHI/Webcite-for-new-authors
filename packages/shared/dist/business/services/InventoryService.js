import { ResourceNotFoundError, BusinessRuleError, DatabaseError } from '../errors';
export class InventoryService {
    ...
    async getInventoryLevels(filters = {}) {
        try {
            const { product_id, category_id, low_stock_only = false, track_inventory_only = true, search, limit = 50, offset = 0 } = filters;
        ...
        `);
            ...
            if (search) {
                query = query.or(`name.ilike.%${search}%,sku.ilike.%${search}%,products.name.ilike.%${search}%`);
            }
            ...
            const enhancedVariants = (variants || []).map(variant => {
                const stockStatus = this.calculateStockStatus(variant.inventory_quantity, variant.low_stock_threshold);
                return {
                    ...variant,
    ...
    async getInventoryAlerts() {
        ...
        `)
            ...
            const alerts = (variants || []).map(variant => {
     ...
     */
    async adjustInventory(request, userId) {
        try {
     ...
     */
    async restockInventory(request, userId) {
        try {
            // Validate variant
     ...
     */
    async getInventoryMovements(variantId, filters = {}) {
        try {
            const { movement_type, reference_type, start_date, end_date, limit = 50, offset = 0 } = filters;
            let query = this.supabase
    ...
    async getInventoryAnalytics(startDate, endDate) {
        ...
        `)
            ...
            const totalStockValue = stockData.reduce((sum, item) => sum + (item.inventory_quantity * item.unit_amount), 0);
            // Stock status analysis
            const totalVariants = stockData.length;
            const lowStockItems = stockData.filter(item => item.inventory_quantity <= item.low_stock_threshold).length;
            const outOfStockItems = stockData.filter(item => item.inventory_quantity <= 0).length;
            ...
            const averageInventory = stockData.reduce((sum, item) => sum + item.inventory_quantity, 0) / totalVariants || 1;
     ...
     */
    async setLowStockThresholds(updates) {
        try {
            const results = [];
            const errors = [];
            for (const update of updates) {
     ...
     */
    async generateInventoryReport(format = 'summary') {
        try {
     ...
     */
    calculateStockStatus(currentQuantity, threshold) {
        if (currentQuantity <= 0) {
            return { status: 'out_of_stock', severity: 'critical' };
        }
        else if (currentQuantity <= threshold) {
            return { status: 'low_stock', severity: 'warning' };
        }
        else {
            return { status: 'in_stock', severity: 'normal' };
        }
    }
    /**
     * Estimate days of stock remaining (simplified calculation)
     */
    estimateDaysOfStock(variantId, currentQuantity) {
     ...
     */
    generateInventoryRecommendations(alerts, analytics, levels) {
        const recommendations = [];
        // Critical stock issues
        const criticalItems = alerts.filter(a => a.severity === 'critical');
        if (criticalItems.length > 0) {
            recommendations.push(`🚨 ${criticalItems.length} items are out of stock and need immediate restocking`);
        }
        // Low stock warnings
        const warningItems = alerts.filter(a => a.severity === 'warning');
        if (warningItems.length > 5) {
            recommendations.push(`⚠️ ${warningItems.length} items are running low on stock`);
        }
        // Stock health
        if (analytics.summary.stock_health_percentage < 70) {
            recommendations.push(`📊 Overall stock health is ${analytics.summary.stock_health_percentage}% - consider reviewing thresholds`);
        }
        // High turnover items
        if (analytics.top_moving_items?.length > 0) {
            const topItem = analytics.top_moving_items[0];
            recommendations.push(`🔄 Monitor high-turnover items closely for potential stockouts`);
        }
        // Excess inventory (items with very high stock levels)
        const excessItems = levels.filter(l => l.inventory_quantity > (l.low_stock_threshold * 10)).length;
        if (excessItems > 0) {
            recommendations.push(`📦 ${excessItems} items may have excess inventory - consider adjusting order quantities`);
        }
