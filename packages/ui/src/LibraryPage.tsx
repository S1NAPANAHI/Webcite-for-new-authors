...
const updateLibraryItemProgress = async (itemId: string, isFinished: boolean): Promise<UserLibraryItem> => {
  const { data, error } = await supabase
    ...
    .update({ is_finished: isFinished, updated_at: new Date().toISOString() })
    .eq('id', itemId)
    .select();
  if (error) throw new Error(error.message);
...
// --- Work Card Component (New Design) ---
export const WorkCard: React.FC<{ work: Work; userLibraryItem?: UserLibraryItem; queryClient: any }> = ({ work, userLibraryItem, queryClient }) => {
  const downloadLimit = 5; // Hardcoded for now
  const { user } = useAuth(); // Get current user for rating
  const [userCurrentRating, setUserCurrentRating] = useState<number | null>(null);
  const [isSampleExpanded, setIsSampleExpanded] = useState(false);
  const [showSample, setShowSample] = useState(false);
  ...
  const updateDownloadMutation = useMutation({
    mutationFn: ({ itemId, format, currentCount }: { itemId: string; format: 'epub' | 'pdf' | 'mobi'; currentCount: number }) => updateDownloadCount(itemId, format, currentCount),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['userLibraryItems'] });
    },
    onError: (err) => {
      alert(`Error updating download count: ${err.message}`);
    }
  });

  const handleDownload = async (format: 'epub' | 'pdf' | 'mobi') => {
    if (!userLibraryItem) {
      alert("Please log in or purchase this item to download.");
      return;
    }
    const url = work[`${format}_url`] as string;
    const downloadsUsed = userLibraryItem[`${format}_downloads_used`];

    if (!url) {
      alert(`No ${format.toUpperCase()} file available for this work.`);
      return;
    }

    if (downloadsUsed >= downloadLimit) {
      alert(`You have exhausted your ${format.toUpperCase()} download limit (${downloadLimit}). Please contact support for a reset.`);
      return;
    }
    ...
    updateDownloadMutation.mutate({ itemId: userLibraryItem.id, format, currentCount: downloadsUsed });
  };
              ...
              <>
                ...
                {work.epub_url && (
                  <button
                    ...
                    className={`btn ${userLibraryItem.epub_downloads_used >= downloadLimit ? 'disabled-btn' : ''}`}
                    disabled={userLibraryItem.epub_downloads_used >= downloadLimit}
                  >
                    EPUB ({userLibraryItem.epub_downloads_used}/{downloadLimit})
                  </button>
                )}
                {work.pdf_url && (
                  <button
                    onClick={() => handleDownload('pdf')}
                    className={`btn ${userLibraryItem.pdf_downloads_used >= downloadLimit ? 'disabled-btn' : ''}`}
                    disabled={userLibraryItem.pdf_downloads_used >= downloadLimit}
                  >
                    PDF ({userLibraryItem.pdf_downloads_used}/{downloadLimit})
                  </button>
                )}
                {work.mobi_url && (
                  <button
                    onClick={() => handleDownload('mobi')}
                    className={`btn ${userLibraryItem.mobi_downloads_used >= downloadLimit ? 'disabled-btn' : ''}`}
                    disabled={userLibraryItem.mobi_downloads_used >= downloadLimit}
                  >
                    MOBI ({userLibraryItem.mobi_downloads_used}/{downloadLimit})
                  </button>
                )}
                <button
                  onClick={() => updateProgressMutation.mutate({ itemId: userLibraryItem.id, isFinished: !userLibraryItem.is_finished })}
